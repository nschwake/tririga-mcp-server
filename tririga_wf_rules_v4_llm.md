# TRIRIGA WORKFLOW CREATION RULES v4
# For LLM Agents Using MCP Tools
# Format: LLM-optimized. Dense. Authoritative. Source-verified against Java decompile.
...
- DB2: `SELECT NEXTVAL FOR SEQ_NAME FROM SYSIBM.SYSDUMMY1`
- Oracle: `SELECT SEQ_NAME.NEXTVAL FROM DUAL`
- PostgreSQL: `SELECT nextval('seq_name')`
- SQL Server: `SELECT NEXT VALUE FOR SEQ_NAME`
...
- R2.3: EXPRESSION MAP_IDs: every EXPRESSION-bearing task gets a NEW MAP_ID from EXPRESSION_SEQ every version (via copyWFCondition). This includes Start task — no special-casing needed.
...
| EXPRESSION_SEQ | EXPRESSION.ID / TASK.MAP_ID | Per EXPRESSION-bearing task per clone (copyWFCondition) |

- R4.1: Never manually assign key values. Always use sequences.
- R4.2: Discover and verify sequence names before first use.
- R4.3: Call each sequence once per logical record. Store result. Never call twice for same record.
...
| 80 | User Defined Formula | SEQ_FORMULA_ID result | function name | SRC_MEMBER_ID = SOBJTYPE_FORMULA_HDR.FORMULA_ID. See Rule 6.8. |
...
- R6.4: MAP_TYPE 80 clone sequence (source-verified from WFTCopyDAO.java + cloneFormula method):
  1. INSERT OTM verbatim (SRC_MEMBER_ID still has old value)
  2. For each MAP_TYPE 80 row: `SELECT NEXTVAL FOR SEQ_FORMULA_ID` → :new_formula_id
  3. `INSERT INTO SOBJTYPE_FORMULA_HDR (SPEC_TEMPLATE_ID, ATR_SEQ, FORMULA_ID, FORMULA, CREATED_BY, UPDATED_BY) SELECT ?, ?, ?, FORMULA, CREATED_BY, UPDATED_BY FROM SOBJTYPE_FORMULA_HDR WHERE SPEC_TEMPLATE_ID=? AND ATR_SEQ=? AND FORMULA_ID=?` — params: (-target_wft_id, new_version, new_formula_id, -src_wft_id, current_max, old_formula_id)
  4. `INSERT INTO SOBJTYPE_FORMULA_PARAMS (SPEC_TEMPLATE_ID, ATR_SEQ, FORMULA_ID, PARAM_ID, PARAM_TYPE, PARAM_NAME, PARAM_STR, PARAM_DISP_STR, PARAM_ATR_SEQ, PARAM_FLD_QRY_FLG, PARAM_QRY_VAL_FLG, CREATED_BY, UPDATED_BY) SELECT ?, ?, ?, PARAM_ID, PARAM_TYPE, PARAM_NAME, PARAM_STR, PARAM_DISP_STR, PARAM_ATR_SEQ, PARAM_FLD_QRY_FLG, PARAM_QRY_VAL_FLG, CREATED_BY, UPDATED_BY FROM SOBJTYPE_FORMULA_PARAMS WHERE SPEC_TEMPLATE_ID=? AND ATR_SEQ=? AND FORMULA_ID=?` — same params pattern
  5. `UPDATE OBJECT_TYPE_MAP SET SRC_MEMBER_ID=? WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=? AND MAP_TYPE=80 AND MAP_ID=? AND SRC_MEMBER_ID=?` — params: (new_formula_id, target_wft_id, new_version, map_id, old_formula_id)
...
- R7.7: Start task MAP_ID points to EXPRESSION record. No OTM rows. New EXPRESSION_SEQ value generated for Start task on every new version (via copyWFCondition — no special handling needed, same as all other EXPRESSION-bearing tasks).
...
### 8.4 Switch Task Clone Rules (within copyWFCondition)
- R8.13: copyWFCondition handles Switch tasks identically to Start/Break — generates new EXPRESSION_SEQ value, copies EXPRESSION + EXPRESSION_PARAM, updates TASK.MAP_ID. No special Switch-specific clone logic needed beyond this.
...
- R8.16: Two formula categories:
  ...
  - When cloning: carry EXPRESSION.FORMULA and EXPRESSION_PARAM verbatim. copyWFCondition handles this. Do not re-validate formula names.
...
- R11.5: Re-parent BEFORE deleting. Verify: `SELECT STEP_ID FROM WF_TEMPLATE_STEP WHERE PARENT_STEP_ID = :removed_step_id` → expect 0 rows.
...
- R11.8: Deletion order for any removed step:
  ```
  0. DELETE TASK_FILTER WHERE TASK_ID=:id, WF_TEMPLATE_ID=:tid, WF_TEMPLATE_VERSION=:ver
  1. DELETE EXPRESSION_PARAM WHERE FORMULA_ID=:map_id  (EXPRESSION-bearing types only)
  2. DELETE EXPRESSION WHERE ID=:map_id               (EXPRESSION-bearing types only)
  3. DELETE SOBJTYPE_FORMULA_PARAMS WHERE SPEC_TEMPLATE_ID=-:tid AND ATR_SEQ=:ver AND FORMULA_ID=:src_member_id (MAP_TYPE 80 only)
  4. DELETE SOBJTYPE_FORMULA_HDR WHERE SPEC_TEMPLATE_ID=-:tid AND ATR_SEQ=:ver AND FORMULA_ID=:src_member_id (MAP_TYPE 80 only)
  5. DELETE OBJECT_TYPE_MAP WHERE MAP_ID=:map_id, WF_TEMPLATE_ID=:tid, WF_TEMPLATE_VERSION=:ver
  6. DELETE TASK WHERE TASK_ID=:id, WF_TEMPLATE_ID=:tid, VERSION=:ver
  7. DELETE WF_TEMPLATE_STEP WHERE STEP_ID=:id, WF_TEMPLATE_ID=:tid, WF_TEMPLATE_VERSION=:ver
  8. DELETE WF_STEP_REF WHERE STEP_ID=:id, WF_TEMPLATE_ID=:tid, WF_TEMPLATE_VERSION=:ver
  ```
...
- R12.1: No fields are auto-populated via MCP. All fields including timestamps/user IDs must be set explicitly.
- R12.2: Before first INSERT, query non-nullable columns with no default for each target table.
- R12.3: For unknown required fields, query existing record of same type as reference. Never fabricate.
...
4. Discover and verify all sequences (Section 4)
5. CHECK: SELECT WF_TEMPLATE_ID, WF_TEMPLATE_VERSION, STATUS_ID FROM WF_TEMPLATE WHERE WF_TEMPLATE_ID=:id ORDER BY WF_TEMPLATE_VERSION DESC
   - If STATUS_ID=20 exists → HALT (Rule 3.4)
   - Get MAX(WF_TEMPLATE_VERSION) WHERE STATUS_ID=10 → :current_max
   - SET :new_version = :current_max + 1
...
### Master Clone SQL Sequence (source-verified: WFTCopyDAO.java)

Execute in this exact order. All use SELECT/INSERT pattern. :src_wft_id and :target_wft_id are identical for new version of same workflow.

```sql
-- STEP 1: WF_TEMPLATE (STATUS_ID=20, timestamps=now, SOURCE_WF_TEMPLATE_ID/VERSION=-1 for REVISE type)
...
SELECT ?, ?, BO_CLASS_TYPE_ID, BO_TYPE_NAME, BO_TYPE_ID, BO_EVENT_NAME, WF_NAME, DESCRIPTION,
  20, ?, ?, ?, CREATED_BY, WF_TYPE, -1, -1, ?, ?, ?, END_DATE,
  SECONDARY_BO_CLASS_TYPE_ID, SECONDARY_BO_TYPE_ID, ASSOCIATION_NAME, IGNORE_MODULE_WF_FLAG,
  LOCK_RECORD_FLAG, PROJECT_ID, PROJECT_NAME, INSTANCE_DATA_FLAG, WF_MODIFIER, OBJECT_LABEL_ID
FROM WF_TEMPLATE WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, now, userId, now, true/*TEMPLATE_FLAG*/, bo_id, now, src_wft_id, current_max)
...
SELECT ?, ?, STEP_ID, STEP_TYPE, PARENT_STEP_ID, CREATED_DATE, CREATED_BY
FROM WF_TEMPLATE_STEP WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)

-- STEP 3: OBJECT_TYPE_MAP (ALL rows verbatim including MAP_TYPE 80 — SRC_MEMBER_ID will be patched in Step 4)
...
SELECT MAP_ID, SRC_OBJECT_TYPE_ID, SRC_TAB_ID, SRC_MEMBER_ID, TARGET_OBJECT_TYPE_ID,
  TARGET_TAB_ID, TARGET_MEMBER_ID, MAP_TYPE, CREATED_DATE, CREATED_BY, FIELD_VALUE, ?, ?, SRC_FIELD_VALUE
FROM OBJECT_TYPE_MAP WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)

-- STEP 4: cloneFormulaMaps — for each MAP_TYPE 80 OTM row (if any exist):
-- 4a: query source MAP_TYPE 80 rows
SELECT MAP_ID, SRC_MEMBER_ID FROM OBJECT_TYPE_MAP
WHERE WF_TEMPLATE_ID=:src_wft_id AND WF_TEMPLATE_VERSION=:current_max AND MAP_TYPE=80;
-- 4b: per row → SEQ_FORMULA_ID → :new_formula_id
-- 4c: INSERT SOBJTYPE_FORMULA_HDR (select from source, substitute SPEC_TEMPLATE_ID/ATR_SEQ/FORMULA_ID)
INSERT INTO SOBJTYPE_FORMULA_HDR (SPEC_TEMPLATE_ID, ATR_SEQ, FORMULA_ID, FORMULA, CREATED_BY, UPDATED_BY)
SELECT ?, ?, ?, FORMULA, CREATED_BY, UPDATED_BY
FROM SOBJTYPE_FORMULA_HDR WHERE SPEC_TEMPLATE_ID=? AND ATR_SEQ=? AND FORMULA_ID=?;
-- params: (-target_wft_id, new_version, new_formula_id, -src_wft_id, current_max, old_src_member_id)
-- 4d: INSERT SOBJTYPE_FORMULA_PARAMS
...
SELECT ?, ?, ?, PARAM_ID, PARAM_TYPE, PARAM_NAME, PARAM_STR, PARAM_DISP_STR, PARAM_ATR_SEQ,
  PARAM_FLD_QRY_FLG, PARAM_QRY_VAL_FLG, CREATED_BY, UPDATED_BY
FROM SOBJTYPE_FORMULA_PARAMS WHERE SPEC_TEMPLATE_ID=? AND ATR_SEQ=? AND FORMULA_ID=?;
-- params: (-target_wft_id, new_version, new_formula_id, -src_wft_id, current_max, old_src_member_id)
-- 4e: UPDATE OTM to replace old SRC_MEMBER_ID with new_formula_id
UPDATE OBJECT_TYPE_MAP SET SRC_MEMBER_ID=?
WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=? AND MAP_TYPE=80 AND MAP_ID=? AND SRC_MEMBER_ID=?;
-- params: (new_formula_id, target_wft_id, new_version, map_id, old_src_member_id)
...
SELECT ?, ?, MAP_ID, GUI_ID, SPEC_TEMPLATE_ID, TAB_NAME, SECTION_NAME, FIELD_NAME,
  PROPERTY_TYPE, PROPERTY_VALUE, MAP_TYPE, TASK_ID, ACTION_ID, TASK_MAP_DATA
FROM GUI_INSTANCE_WF_MAP WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)

-- STEP 6: TASK (all rows verbatim — MAP_IDs still carry old values; copyWFCondition patches them in Step 7)
...
SELECT TASK_ID, ?, TASK_LABEL, ?, CLASSIFICATION_TYPE_ID, OBJECT_TYPE_ID, EVENT_ACTION,
  DESCRIPTION, ASSIGN_TO_USER, EST_END_IN_DAYS, EST_END_IN_MINS, EST_END_IN_HRS, RECURRENCE_ID,
  MAP_ID, SOURCE_TASK_ID, ASSIGN_TO_FLAG, TARGET_TASK_ID, SERVICE_CONTEXT, SERVICE_ASSOC_STRING,
  FILTER_CLASS, FILTER_OBJECT, FILTER_SECTION, FILTER_FIELD, FILTER_OPERATOR, FILTER_VALUE,
  SUM_SECTION, SUM_FIELD, ASSIGNEE_TASK_ID, TARGET_ASSOC_STRING, TASK_TYPE, LOCK_USER,
  ASSOC_CLASS_TYPE_ID, ASSOC_OBJECT_TYPE_ID, DELETE_SECTION, USE_MAP, SORT_COUNT, DATE_CONTEXT,
  TRANSACTION_TYPE, STATUS, PARAMS_MAP_ID, FORMULA_RECALC
FROM TASK WHERE WF_TEMPLATE_ID=? AND VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)

-- STEP 7: copyWFCondition — two-phase (source-verified: WFTCopyDAO.java + ExpressionDatabaseRepr.java)
-- Phase 7a: query source EXPRESSION-bearing tasks
SELECT MAP_ID, TASK_ID FROM TASK
WHERE WF_TEMPLATE_ID=:src_wft_id AND VERSION=:current_max
...
-- Phase 7b: for EACH row returned:
--   7b-i:  EXPRESSION_SEQ → :new_map_id
--   7b-ii: INSERT INTO EXPRESSION (FORMULA, ID) SELECT FORMULA, :new_map_id FROM EXPRESSION WHERE ID=:old_map_id
--   7b-iii: INSERT INTO EXPRESSION_PARAM (FORMULA_ID, PARAM_ID, PARAM_STR) SELECT :new_map_id, PARAM_ID, PARAM_STR FROM EXPRESSION_PARAM WHERE FORMULA_ID=:old_map_id
--   7b-iv: UPDATE TASK SET MAP_ID=:new_map_id WHERE WF_TEMPLATE_ID=:target_wft_id AND VERSION=:new_version AND TASK_ID=:task_id
...
SELECT ?, ?, STEP_ID, USAGE_TYPE, REF_TYPE, TASK_REF_TYPE, CONTEXT_TYPE, RECORD_ID,
  REF_TASK_ID, MODULE_ID, OBJECT_TYPE_ID, SECTION, FIELD, ASSOC
FROM WF_STEP_REF WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)
...
SELECT REMINDER_ID, TASK_ID, SUBSCRIBER_ID, DAYS, HOURS, MINUTES, REMINDER_DATETIME,
  CREATED_BY, TASK_DUE_DATETIME, REMINDER_TYPE, ACTIVE_FLAG, NOTE_ID, ?, ?
FROM TASK_REMINDER WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)
...
SELECT TASK_ID, ?, ?, SEQUENCE_ID, LEFT_SECTION_NAME, LEFT_FIELD_NAME, LEFT_DATA_TYPE,
  LEFT_LIST_FLAG, OPERATOR, RIGHT_LIST_TYPE, RIGHT_SECTION_NAME, RIGHT_FIELD_NAME,
  RIGHT_DATA_TYPE, CONSTANT_FLAG, RIGHT_VALUE, RIGHT_LIST_FLAG, ASSOCIATED_TO_TASK,
  ASSOCIATED_TO_CHILDREN, ASSOCIATED_TO_NAME
FROM TASK_FILTER WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)
...
SELECT NOTE_ID, POSITION, TEXT, PROJECT_ID, OBJECT_ID, OBJECT_TYPE_ID, NOTETYPE,
  REVISION, AUTHOR_ID, CREATE_DATE, UPDATED_BY, UPDATED_DATE, ?, ?
FROM NOTE WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)
...
FROM ATTACHMENT WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)
...
SELECT PARAMS_MAP_ID, ?, ?, PARAM_ID, TYPE, VAR_ID, VAR_MODULE_ID, VAR_BO_ID, NAME
FROM WF_PARAMS_MAP WHERE WF_TEMPLATE_ID=? AND WF_TEMPLATE_VERSION=?;
-- params: (target_wft_id, new_version, src_wft_id, current_max)
```
...
- R16.2: Use parameterized queries. Never interpolate user-supplied values into SQL strings.
- R16.3: Log every query executed and its row count. Surface full log to user at session end.
...
- R16.6: INSERT and UPDATE MCP tools are not yet available. Flag this limitation at start of any write session.
...
- R16.8: Never fabricate values for any column. Query existing same-type record as reference when uncertain.
- R16.9: When creating any new step, collect TASK_LABEL from user and optionally DESCRIPTION.
- R16.10: All clone-phase operations use SELECT/INSERT pattern as defined in Section 15. Never specify individual column values for cloned rows.
